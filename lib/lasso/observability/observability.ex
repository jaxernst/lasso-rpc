defmodule Lasso.RPC.Observability do
  ...
  def build_client_metadata(%RequestContext{} = ctx) do
    metadata = %{
      ...
      profile_id: ctx.opts && ctx.opts.profile,
      service_profile_id: ctx.opts && ctx.opts.profile,
      head_policy: Lasso.RPC.HeadPolicy.metadata(ctx),
      ...
      executed_channel: ctx.executed_channel,
      attempted_channels: ctx.attempted_channels,

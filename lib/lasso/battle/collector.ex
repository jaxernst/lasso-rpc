defmodule Lasso.Battle.Collector do
  @moduledoc """
  ...
  defp attach_collector(:requests) do
    ...
    Logger.debug(
      "Attached request collectors: prod=#{inspect(prod_handler_id)}, battle=#{inspect(battle_handler_id)}"
    )
  ...
  defp handle_request_event(_event_name, measurements, metadata, _config) do
    ...
    try do
      Agent.update(__MODULE__, fn data ->
        ...
        Logger.debug(
          "Collector Agent update: #{length(requests)} -> #{length(requests) + 1} requests"
        )

    ...
    <div class="fixed bottom-2 left-2 z-40 hidden items-center gap-2 lg:flex">
      <div class={[
        ...
        <span class="text-gray-300">
          <%= if @total == 1 do %>
            <%= if @responding == 1 do %>
              <span class={status_text_color(compute_status(@responding, @total))}>
                Standalone node
              </span>
            <% else %>
              <span class={status_text_color(compute_status(@responding, @total))}>
            ...
            <% end %>
          <% else %>
            <span class={status_text_color(compute_status(@responding, @total))}><%= @responding %></span>/{@total} cluster nodes reporting
          <% end %>
        </span>

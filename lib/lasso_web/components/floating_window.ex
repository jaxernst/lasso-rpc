defmodule LassoWeb.Components.FloatingWindow do
  ...
  attr :position, :atom, default: :top_right, values: [:top_left, :top_right, :bottom_left, :bottom_right, :center]
  ...
  def floating_window(assigns) do
    ...
    {collapsed_arrow, expanded_arrow} = case assigns.position do
      :top_left -> {"↘", "↖"}
      :top_right -> {"↙", "↗"}
      :bottom_left -> {"↗", "↙"}
      :bottom_right -> {"↖", "↘"}
  ...
  def window_header(assigns) do
    ~H"""
    <div class={[
      "border-gray-700/50 flex items-center justify-between border-b px-3 py-2",
      @class
    ]}>
  ...
  def window_footer(assigns) do
    ~H"""
    <div class={[
      "border-gray-700/50 border-t p-3",
      @class
    ]}>
      <%= render_slot(@inner_block) %>
    </div>
  ...
  def window_section(assigns) do
    border_class = case assigns.border do
      :none -> ""
  ...
  def metric_card(assigns) do
    ~H"""
    <div class={["bg-gray-800/50 rounded-lg p-3 text-center overflow-hidden", @class]}>
      <div class="text-[11px] leading-tight text-gray-400 truncate"><%= @label %></div>
  ...
  def metrics_grid(assigns) do
    grid_cols = case assigns.cols do
      2 -> "grid-cols-2"
      3 -> "grid-cols-3"
  ...
  def event_feed(assigns) do
    ...
    <div
      ...
      class={[
        "flex flex-col-reverse gap-1 overflow-y-auto",
        @max_height,
        @class
      ]}
    >
  ...
  def action_group(assigns) do
    align_class = case assigns.align do
      :left -> "justify-start"
      :right -> "justify-end"

defmodule LivechainWeb.Admin.ChainController do
  ...
  def create(conn, %{"chain" => chain_params}) do
    with :ok <- validate_chain_params(chain_params),
      ...
      PubSub.broadcast(Livechain.PubSub, "chain_config_updates", 
          ...
          case ConfigValidator.test_provider_connectivity(provider) do
            :ok ->
              %{
                ...
                url: provider.url,
                status: "connected",
                response_time_ms: nil  # Could be enhanced to measure actual response time
              }
            {:error, reason} ->
              %{
                provider_id: provider.id,
  ...
  defp format_error({:invalid_chain_config, error}), do: "Invalid configuration: #{inspect(error)}"
  defp format_error({:yaml_generation_failed, reason}), do: "YAML generation failed: #{inspect(reason)}"
  defp format_error({:file_write_failed, reason}), do: "File write failed: #{inspect(reason)}"
  defp format_error({:config_reload_failed, reason}), do: "Config reload failed: #{inspect(reason)}"

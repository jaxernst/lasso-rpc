defmodule Livechain.RPC.ChainSupervisor do
  ...
  alias Livechain.RPC.{WSConnection, ProviderPool, TransportRegistry}
  alias Livechain.RPC.ProviderSupervisor
  ...
  def ensure_provider(chain_name, provider_config, opts \\ []) do
    with {:ok, chain_config} <- get_chain_config(chain_name),
         ...
         :ok <- start_provider_supervisor(chain_name, chain_config, provider_config, opts) do
      :ok
    else
      {:error, reason} = error ->
        Logger.error(
  ...
  defp provider_supervisor_via(chain_name, provider_id) do
    {:via, Registry, {Livechain.Registry, {:provider_supervisor, chain_name, provider_id}}}
  end

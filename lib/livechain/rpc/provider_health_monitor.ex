defmodule Livechain.RPC.ProviderHealthMonitor do
  ...
  alias Livechain.RPC.{HttpClient, CircuitBreaker, ProviderPool}
  ...
  def via(chain), do: {:via, Registry, {Livechain.Registry, {:provider_health_monitor, chain}}}
  ...
  defp check_provider(chain, provider) do
    ...
    case result do
      {:ok, %{"jsonrpc" => "2.0"} = response} ->
        case Livechain.RPC.Normalizer.run(provider.id, "eth_chainId", response) do
          {:ok, _} ->
            ProviderPool.report_success(chain, provider.id)

          {:error, %Livechain.JSONRPC.Error{} = jerr} ->
      ...
      {:ok, other} ->
        ...
        jerr =
          Livechain.JSONRPC.Error.from({:response_decode_error, inspect(other)},
      ...
      {:error, reason} ->
        jerr = Livechain.JSONRPC.Error.from(reason, provider_id: provider.id)
        ProviderPool.report_failure(chain, provider.id, {:health_check, jerr})
    end
  end

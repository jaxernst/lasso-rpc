defmodule Livechain.RPC.ChainManager do
  ...
  def handle_call({:start_all_chains, config_path}, _from, state) do
    case ChainConfig.load_config(config_path) do
      {:ok, config} ->
        ...
        Logger.info("Successfully started #{successful_starts}/#{map_size(config.chains)} chain supervisors")
  ...
  defp schedule_stats_update do
    Process.send_after(self(), :update_stats, 10_000)  # Update every 10 seconds
  end
end
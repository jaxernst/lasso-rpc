defmodule Livechain.RPC.Normalizer do
  @moduledoc """
  ...
  def run(provider_id, method, response, opts) when is_map(response) do
    adapter = Keyword.get(opts, :adapter, Livechain.RPC.Adapters.Generic)
  ...
  defp emit_result_telemetry(provider_id, method, status) do
    :telemetry.execute([:livechain, :normalize, :result], %{count: 1}, %{
  ...
  defp emit_error_telemetry(provider_id, method, %JError{} = jerr) do
    :telemetry.execute([:livechain, :normalize, :error], %{count: 1}, %{
      provider_id: provider_id,
      method: method,
      code: jerr.code,

defmodule Livechain.RPC.ClientSubscriptionRegistry do
  @moduledoc """
  ...
  def via(chain), do: {:via, Registry, {Livechain.Registry, {:client_registry, chain}}}
  ...
  def handle_call({:add, subscription_id, client_pid, key}, _from, state) do
    ...
    :telemetry.execute([:livechain, :subs, :client_subscribe], %{count: 1}, %{
      chain: state.chain,
  ...
  def handle_call({:remove, subscription_id}, _from, state) do
    case Map.pop(state.by_id, subscription_id) do
      ...
      {%{key: key}, new_by_id} ->
        ...
        :telemetry.execute([:livechain, :subs, :client_unsubscribe], %{count: 1}, %{
          chain: state.chain,
          subscription_id: subscription_id
        })

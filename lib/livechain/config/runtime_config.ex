defmodule Livechain.Config.RuntimeConfig do
  @moduledoc """
  ...
  def get_circuit_breaker_config do
    ...
    user_config = Application.get_env(:livechain, :circuit_breaker, %{})
  ...
  defp normalize_chain_config(config) do
    %{
      ...
      failover: Map.get(config, :failover, %{
        enabled: true,
        max_backfill_blocks: 32,
        backfill_timeout: 30_000
      }),
      dedupe: Map.get(config, :dedupe, %{
        max_items: 256,
        max_age_ms: 30_000
      })
    }

defmodule Livechain.Config.ConfigValidator do
  ...
  def format_error(:missing_url_scheme), do: "Provider URL must include a scheme (http/https/ws/wss)"
  ...
  defp validate_provider_structures(providers) do
    invalid_provider = Enum.find(providers, fn provider ->
  ...
  defp validate_provider_connectivity(providers) do
    ...
    failed_providers = Enum.filter(results, fn {_id, result} ->

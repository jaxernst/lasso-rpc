defmodule Livechain.Simulator do
  ...
  def handle_cast(:start_simulation, state) do
    ...
    event_timer = if state.config.enable_event_broadcasting do
  ...
  def handle_info({:reconnect, _old_connection_id, chain_name}, state) do
    ...
    # Create new endpoint
    endpoint = chain_fn.([id: new_connection_id, name: "Sim #{String.capitalize("#{chain_name}")} (Reconnected)"])
    ...
    case WSSupervisor.start_connection(endpoint) do
      {:ok, _pid} ->
        ...
        timer_ref = Process.send_after(self(), {:connection_lifecycle, new_connection_id}, lifetime)
  ...
  defp schedule_next_spawn(config) do
    interval = config.min_spawn_interval + :rand.uniform(config.max_spawn_interval - config.min_spawn_interval)

defmodule Livechain.Testing.MockWSProvider do
  @moduledoc """
  ...
  def start_mock(chain, spec) when is_map(spec) do
    ...
    {:ok, pid} =
      GenServer.start_link(
        __MODULE__,
        {chain, spec},
        name: {:via, Registry, {Livechain.Registry, {:ws_conn, provider_id}}}
      )
    ...
    with :ok <- ensure_chain_exists(chain),
         :ok <- Livechain.Config.ConfigStore.register_provider_runtime(chain, provider_config),
         :ok <- Livechain.RPC.ProviderPool.register_provider(chain, provider_id, provider_config) do
      # Mark as healthy so it can be selected
      Livechain.RPC.ProviderPool.report_success(chain, provider_id)
  ...
  def stop_mock(chain, provider_id) do
    ...
    Livechain.Config.ConfigStore.unregister_provider_runtime(chain, provider_id)
  ...
  defp ensure_chain_exists(chain_name) do
    case Livechain.Config.ConfigStore.get_chain(chain_name) do
      ...
      {:error, :not_found} ->
        ...
        with :ok <-
               Livechain.Config.ConfigStore.register_chain_runtime(chain_name, default_config),
             {:ok, chain_config} <- Livechain.Config.ConfigStore.get_chain(chain_name),
             {:ok, _pid} <-

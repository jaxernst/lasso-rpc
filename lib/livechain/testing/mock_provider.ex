defmodule Livechain.Testing.MockProvider do
  @moduledoc """
  ...
  via `Livechain.Providers.add_provider/3`. Responses are realistic and deterministic.
  ...
  def start_mock(chain, spec) when is_map(spec) do
    ...
    case Plug.Cowboy.http(__MODULE__, [mock_config: config],
      ...
      {:ok, pid} ->
        ...
        case Livechain.Config.ConfigStore.register_provider_runtime(chain, provider_config) do
          :ok ->
            # Now register with ChainSupervisor/ProviderPool
            case Livechain.Providers.add_provider(chain, provider_config) do
              {:ok, ^provider_id} ->
                ...
                case Livechain.RPC.TransportRegistry.get_channel(chain, provider_id, :http) do
                  {:ok, _channel} ->
  ...
  def stop_mock(chain, provider_id) do
    ...
    Livechain.Providers.remove_provider(chain, provider_id)

defmodule Livechain.Battle.WebSocketClient do
  @moduledoc """
  ...
  def handle_frame({:text, msg}, state) do
    case Jason.decode(msg) do
      {:ok, %{"id" => _id, "result" => subscription_id}} when is_binary(subscription_id) ->
        ...
        :telemetry.execute(
          [:livechain, :battle, :ws_subscription, :created],
          %{count: 1},
          %{
  ...
  def handle_disconnect(disconnect_map, state) do
    ...
    :telemetry.execute(
      [:livechain, :battle, :ws_subscription, :disconnected],
      %{count: 1},
  ...
  defp handle_subscription_event(%{"subscription" => _sub_id, "result" => result}, state) do
    ...
    # Emit telemetry
    :telemetry.execute(
      [:livechain, :battle, :ws_subscription, :event],
      %{count: 1},
      %{
        client_id: state.client_id,
        subscription_id: state.subscription_id,

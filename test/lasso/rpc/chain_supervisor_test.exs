defmodule Lasso.RPC.ChainSupervisorTest do
  @moduledoc """
  ...
  # Default profile for test chains
  ...
  describe "Supervisor Lifecycle" do
    test "starts and initializes all child processes", %{chain_config: chain_config} do
      ...
      assert Enum.any?(children, fn {mod, _pid, _type, _} ->
               mod == {Lasso.RPC.ProviderPool, {chain_name, chain_config}}
             end) or
    ...
    test "handles startup with invalid configuration gracefully" do
      ...
      _log =
        capture_log(fn ->
          ...
          result = ChainSupervisor.start_link({@test_profile, "invalid_chain", invalid_config})
          Process.sleep(100)

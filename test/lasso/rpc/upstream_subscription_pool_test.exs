defmodule Lasso.RPC.UpstreamSubscriptionPoolTest do
      ...
      :sys.replace_state(UpstreamSubscriptionPool.via(chain), fn state ->
        ...
        %{
          ...
          | keys: Map.put(state.keys, key, entry),
            pending_subscribe:
              Map.put(state.pending_subscribe, request_id, {@test_provider, key, timestamp})
        }
      end)
    ...
    test "handles provider mismatch in confirmation", %{chain: chain} do
      ...
      :sys.replace_state(UpstreamSubscriptionPool.via(chain), fn state ->
        ...
        %{
          ...
          | keys: Map.put(state.keys, key, entry),
            pending_subscribe:
              Map.put(state.pending_subscribe, request_id, {"provider_a", key, timestamp})
        }
      end)
  ...
  describe "Late confirmation handling" do
    test "cleans up orphaned upstream subscription when confirmation arrives late", %{
      chain: chain
    } do
      key = {:newHeads}
      request_id = "req_123"
    ...
    test "cleans up stale pending subscriptions after timeout", %{chain: chain} do
      ...
      :sys.replace_state(UpstreamSubscriptionPool.via(chain), fn state ->
        ...
        %{
          ...
          | keys: Map.put(state.keys, key, entry),
            pending_subscribe:
              Map.put(state.pending_subscribe, request_id, {@test_provider, key, old_timestamp})
        }
      end)
          ...
          | keys: Map.put(state.keys, key, entry),
            pending_subscribe:
              Map.put(
                ...
                request_id,
                {@test_provider, key, recent_timestamp}
              )
        }
      end)

defmodule Lasso.Core.Streaming.UpstreamSubscriptionManagerTest do
  ...
  describe "subscription lifecycle" do
    test "first consumer creates new upstream subscription", %{
      chain: chain,
      provider: provider,
      profile: profile
    } do
      ...
      # First consumer
      {:ok, status} =
        UpstreamSubscriptionManager.ensure_subscription(profile, chain, provider, key)
    ...
    test "second consumer joins existing subscription", %{
      chain: chain,
      provider: provider,
      profile: profile
    } do
    ...
    test "new consumer cancels pending teardown", %{
      chain: chain,
      provider: provider,
      profile: profile
    } do
  ...
  describe "event dispatching" do
    test "events dispatched to all registered consumers", %{
      chain: chain,
      provider: provider,
      profile: profile
    } do
  ...
  describe "provider disconnect handling" do
    test "provider disconnect cleans up subscriptions for that provider", %{
      ...
      {:ok, _status1} =
        UpstreamSubscriptionManager.ensure_subscription(profile, chain, provider, key1)

      {:ok, _status2} =
        UpstreamSubscriptionManager.ensure_subscription(profile, chain, provider, key2)
  ...
  describe "multiple subscription keys" do
    test "different keys managed independently", %{
      chain: chain,
      provider: provider,
      profile: profile
    } do
  ...
  describe "stale subscription detection (race condition fix)" do
    ...
    test "returns error when connection state is unknown", %{chain: chain, profile: profile} do
      ...
      result =
        UpstreamSubscriptionManager.ensure_subscription(profile, chain, unknown_provider, key)

      assert result == {:error, :connection_unknown}
    end

    test "returns error when provider is disconnected", %{
      chain: chain,
      provider: provider,
      profile: profile
    } do

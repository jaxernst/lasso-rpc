defmodule Lasso.Core.Streaming.UpstreamSubscriptionRegistryTest do
  ...
  defp unique_key do
    ...
    {@default_profile, "test_chain_#{suffix}", "test_provider_#{suffix}", {:newHeads}}
  end
  ...
  describe "registration" do
    test "register_consumer creates registration" do
      ...
      assert :ok = UpstreamSubscriptionRegistry.register_consumer(profile, chain, provider, key)
      assert UpstreamSubscriptionRegistry.count_consumers(profile, chain, provider, key) == 1

      # Cleanup
      UpstreamSubscriptionRegistry.unregister_consumer(profile, chain, provider, key)
    end
    ...
    test "unregister_consumer removes registration" do
      ...
      :ok = UpstreamSubscriptionRegistry.register_consumer(profile, chain, provider, key)
      assert UpstreamSubscriptionRegistry.count_consumers(profile, chain, provider, key) == 1

      :ok = UpstreamSubscriptionRegistry.unregister_consumer(profile, chain, provider, key)
      assert UpstreamSubscriptionRegistry.count_consumers(profile, chain, provider, key) == 0
    end
  end
  ...
  describe "lookup" do
    test "lookup_consumers returns all registered consumers" do
      ...
      consumers = UpstreamSubscriptionRegistry.lookup_consumers(profile, chain, provider, key)
    ...
    test "has_consumers? returns correct boolean" do
      ...
      refute UpstreamSubscriptionRegistry.has_consumers?(profile, chain, provider, key)

      :ok = UpstreamSubscriptionRegistry.register_consumer(profile, chain, provider, key)
      assert UpstreamSubscriptionRegistry.has_consumers?(profile, chain, provider, key)
  ...
  describe "dispatch" do
    test "dispatch sends message to all consumers" do
      ...
      # Dispatch message
      :ok = UpstreamSubscriptionRegistry.dispatch(profile, chain, provider, key, test_message)
  ...
  describe "isolation" do
    test "different keys are isolated" do
      ...
      :ok = UpstreamSubscriptionRegistry.register_consumer(profile, chain, provider, key1)
      :ok = UpstreamSubscriptionRegistry.register_consumer(profile, chain, provider, key2)

      assert UpstreamSubscriptionRegistry.count_consumers(profile, chain, provider, key1) == 1
      assert UpstreamSubscriptionRegistry.count_consumers(profile, chain, provider, key2) == 1

      # Unregister one doesn't affect the other
      :ok = UpstreamSubscriptionRegistry.unregister_consumer(profile, chain, provider, key1)
      assert UpstreamSubscriptionRegistry.count_consumers(profile, chain, provider, key1) == 0
      assert UpstreamSubscriptionRegistry.count_consumers(profile, chain, provider, key2) == 1

      # Cleanup
      :ok = UpstreamSubscriptionRegistry.unregister_consumer(profile, chain, provider, key2)
    end
    ...
    test "different providers are isolated" do
      ...
      :ok = UpstreamSubscriptionRegistry.register_consumer(profile, chain, provider1, key)
      :ok = UpstreamSubscriptionRegistry.register_consumer(profile, chain, provider2, key)

      assert UpstreamSubscriptionRegistry.count_consumers(profile, chain, provider1, key) == 1
      assert UpstreamSubscriptionRegistry.count_consumers(profile, chain, provider2, key) == 1

      # Cleanup
      :ok = UpstreamSubscriptionRegistry.unregister_consumer(profile, chain, provider1, key)
      :ok = UpstreamSubscriptionRegistry.unregister_consumer(profile, chain, provider2, key)
    end
  end
end

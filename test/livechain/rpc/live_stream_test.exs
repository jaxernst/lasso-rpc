defmodule Livechain.RPC.LiveStreamTest do
  ...
  describe "Base Network Live Streaming" do
    ...
    test "streams new blocks from Base mainnet using public RPC" do
      ...
      logs_filter = %{
        # USDC on Base
        "address" => ["0xA0b86a33E6417c8a8aB9d74D46DBF3B6bbea5BBE"],
        "topics" => [
          # Transfer event
    ...
    if current_time < end_time do
      ...
      receive do
        %{event: "new_block", payload: block_data} = message ->
          Logger.info(
            "🧱 New block received on #{extract_chain_name(message)}: #{block_data["number"]}"
          )
      ...
      after
        timeout ->
          if event_count == 0 do
            Logger.info(
              "⏱️  No events received in #{original_seconds} seconds (this is normal for some chains)"
            )
          else

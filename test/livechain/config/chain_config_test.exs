defmodule Livechain.Config.ChainConfigTest do
  ...
  describe "configuration loading" do
    test "load_config/1 successfully loads chains.yml" do
      ...
      assert is_struct(config, ChainConfig)
  ...
  describe "chain configuration retrieval" do
    ...
    test "get_chain_config/2 returns config for existing chain", %{config: config} do
      ...
      assert is_struct(ethereum_config, ChainConfig.ChainConfig)
  ...
  describe "provider management" do
    ...
    test "get_available_providers/1 returns providers with valid URLs", %{ethereum_config: chain_config} do
    ...
    test "get_provider_by_id/2 returns error for non-existent provider", %{ethereum_config: chain_config} do
      assert {:error, :provider_not_found} = ChainConfig.get_provider_by_id(chain_config, "nonexistent")
    end
  end

  describe "configuration validation" do
    test "validate_chain_config/1 accepts valid config" do
      valid_config = %ChainConfig.ChainConfig{
        chain_id: 1,
        name: "Test Chain",
        block_time: 1000,
        providers: [
        ...
        connection: %ChainConfig.ChainConfig.Connection{
          heartbeat_interval: 15000,
          reconnect_interval: 2000,
          max_reconnect_attempts: 10,
          subscription_topics: ["newHeads"]
        },
        aggregation: %ChainConfig.ChainConfig.Aggregation{

...
const CLIENT_NODES = [
  ...
  { lat: 64.13, lng: -21.90, label: "Reykjavik" },
...
class GlobeRenderer {
  constructor(canvas, options = {}) {
    ...
    this.providers = PROVIDER_NODES.map(n => ({
      ...n, pos: latLngTo3D(n.lat, n.lng, 1.0), type: "provider",
    }));
    this.hubs = HUB_NODES.map(n => ({
      ...n, pos: latLngTo3D(n.lat, n.lng, 1.0), type: "hub",
    }));

    this.hubLinks = [];
    for (let i = 0; i < HUB_NODES.length; i++) {
      for (let j = i + 1; j < HUB_NODES.length; j++) {
        this.hubLinks.push(buildArcPath(
  ...
  project(p) {
    const angle = this.rotation + this.currentRotOffset + this.scrollRotationOffset;
  ...
  spawnLifecycle() {
    ...
    for (const hub of HUB_NODES) {
      ...
      if (d < bestDist) { bestDist = d; nearest = hub; }
    }
  ...
  startAnimation() {
    ...
    const animate = (time) => {
      ...
      this.currentRotOffset += (this.targetRotOffset - this.currentRotOffset) * 0.04;
        ...
        for (const arc of lc.fanoutArcs) {
          if (arc.pingFlash > 0) {
            ...
            this.providerPingFlashes[key] = Math.max(
              this.providerPingFlashes[key] || 0, arc.pingFlash
            );
          }
        }
      }
  ...
  drawAtmosphere(ctx) {
    const g = ctx.createRadialGradient(
      this.cx, this.cy, this.radius * 0.8,
      this.cx, this.cy, this.radius * 1.6
    );
  ...
  drawHubLink(ctx) {
    ...
    for (const link of this.hubLinks) {
      ...
      for (let i = 0; i < link.length; i++) {
        ...
        if (!proj.visible) { prevProj = null; continue; }
        if (prevProj) {
          ctx.beginPath();
  ...
  drawNodes(ctx) {
    ...
    for (const node of allNodes) {
      ...
      if (type === "hub") {
        ...
        const g = ctx.createRadialGradient(proj.x, proj.y, 0, proj.x, proj.y, glowR * 0.7);
        g.addColorStop(0, `rgba(52, 211, 153, ${greenAlpha * proj.scale})`);
        g.addColorStop(0.5, `rgba(52, 211, 153, ${(0.12 + flash * 0.3) * proj.scale})`);
  ...
  drawArcs(ctx) {
    for (const lc of this.lifecycles) {
      ...
      if (lc.phase === "inbound" || lc.phase === "fanout" || lc.phase === "fading") {
  ...
  drawArcTrail(ctx, points, progress, fade) {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      if (alpha < 0.01) { prevProj = proj; continue; }
      ...
      if (trail > 0.7) {
        ...
        ctx.fill();
        const hg = ctx.createRadialGradient(proj.x, proj.y, 0, proj.x, proj.y, 10 * proj.scale);
  ...
  drawRim(ctx) {
    const g = ctx.createRadialGradient(
      this.cx, this.cy, this.radius * 0.92,
      this.cx, this.cy, this.radius * 1.1
    );
